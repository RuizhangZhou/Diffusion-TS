...
class IkaDataset(Dataset):
    def __init__(
        ...
        super(IkaDataset, self).__init__()
        assert period in ['train', 'test'], 'period must be train or test.'
        if period == 'train':
            assert ~(predict_length is not None or missing_ratio is not None), ''
        self.name, self.pred_len, self.missing_ratio = name, predict_length, missing_ratio
        self.style, self.distribution, self.mean_mask_length = style, distribution, mean_mask_length
        self.rawdata, self.scaler = self.read_data(data_root)
        self.dir = os.path.join(output_dir, 'samples')
        os.makedirs(self.dir, exist_ok=True)

        self.window, self.period = window, period
        #self.len, self.var_num = self.rawdata.shape[0], self.rawdata.shape[-1]
        self.var_num = self.rawdata.shape[-1]-1
        
        #self.var_num = len(rawdata.columns) - 1
        #self.sample_num_total = max(self.len - self.window + 1, 0)
        self.uniq_id = np.unique(self.rawdata[:, 0])
        self.sample_num_total=len(self.uniq_id)
        self.save2npy = save2npy
        self.auto_norm = neg_one_to_one

        #self.data = self.__normalize(self.rawdata)
        train, inference = self.__getsamples(self.rawdata, proportion, seed)
    ...
    def __getsamples(self, rawdata, proportion, seed):
        x = np.empty([self.sample_num_total, self.window, self.var_num])
        for i in range(self.sample_num_total):
            # start = i
            # end = i + self.window
            # x[i, :, :] = data[start:end, :]
            cur_rawdata = rawdata[rawdata[:, 0] == self.uniq_id[i]]
            cur_rawdata=self.__normalize(cur_rawdata[:, 1:])
            cur_no = len(cur_rawdata)
            if cur_no >= self.window:
                # 从随机选择的起始点开始，截取长度为window的数据片段
                start_idx = np.random.randint(0, cur_no - self.window + 1)
                x[i, :, :] = cur_rawdata[start_idx:start_idx + self.window]
            else:
                x[i, :cur_no, :] = cur_rawdata  # Shape: [1, window, var_num]
            #修剪或填充至max_seq_len

            #x[i,:,:] = data[data[:, 0] == self.uniq_id[i]][:,1:]
